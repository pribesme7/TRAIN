...
def analyse_phi(outfile,philist,step,filemodify,ip):
	...
	for phi in philist:
	    ...
	    subst = 'phi_IR5              :=   {st} ;'.format(st = phi )
            pattern = 'phi_IR5              :=   {st} ;'.format(st = phi - step)
...
def compute_angle(bets,ip):
	...
	alpha1 = np.arctan2((y1[marker1.index('"MKIP1PL1"')]-y2[marker2.index('"MKIP1PL1"')]),x1[marker1.index('"MKIP1PL1"')]-x2[marker2.index('"MKIP1PL1"')])*180./np.pi
	...
	alpha5 = np.arctan2((y1[marker1.index('"MKIP5PL1"')]-y2[marker2.index('"MKIP5PL1"')]),x1[marker1.index('"MKIP5PL1"')]-x2[marker2.index('"MKIP5PL1"')])*180./np.pi
...
def analyse_onsep(outfile,philist,step,filemodify,ip):
	for phi in philist:
	    ...
	    subst = 'on_sep2              :=  %.3f ;'% (phi)
            pattern = 'on_sep2              :=  %.3f ;'% (phi- step ) 
...
def main():
	...
	if init == True:
		...
		#print(phi)
		outfilename = "{pr}_{fs}_anglescan_noheadon_ip15_{sip}.dat".format(pr=prc,fs=fillsch,sip=ip)
		outfile = open(outfilename,'w')
           	phi = np.arange(90,40,-5,dtype = 'int')
		onsep = np.arange(0.138,0.000,-0.01)
		analyse_phi(outfile,phi,step,filemodify,ip)
