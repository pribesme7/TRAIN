...
def analyse_onsep(outfile,outfile2,onseprange,start,step,filename,prc,stg):
    ...
    for onsep in onseprange:
	    ...
	    subst = "on_sep5              := %1.6f ;" % (onsep - step)
            pattern = "on_sep5              := %1.6f ;" % (onsep)
            print(subst)
            print(pattern)
...
#        outfile2.write('# Linear coupling backward beam \n')
        analyse_onsep(outfile,outfile2,onsep,start,step,filename,prc,stg)
